...
### Testing CUDA methods
u0 = ode.u0
u0_mat = zeros(Float64,length(u0), 4)
for i in eachindex(u0)
    u0_mat[i,:] = u0[i]
end
u0_mat_d = CuArray(u0_mat)

using LinearAlgebra
using SparseArrays
using CUDA.CUSPARSE
rbf_differentiation_matrices = semi.cache.rbf_differentiation_matrices
rbf_differentiation_matrices_d = CuSparseMatrixCSC.(rbf_differentiation_matrices)
flux_values = copy(u0_mat_d)
du = copy(u0_mat_d)
du .= 0.0
i = 1
@time apply_to_each_field(mul_by_accum!(rbf_differentiation_matrices_d[i],
                                          -1),
                            du, flux_values)
i = 2
@time apply_to_each_field(mul_by_accum!(rbf_differentiation_matrices_d[i],
                                          -1),
                            du, flux_values)
@which mul!(du, rbf_differentiation_matrices_d[i], flux_values, -1, One())
